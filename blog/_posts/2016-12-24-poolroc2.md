...
Finally, we can map each pair of probability estimates for observations in the test set to the path, and create a ordered list for test set observations. The ROC plot shows that my approach does improve individual model estimates. Although the improvement is marginal, as I repeat the experiment in the post it repeatly appears, which is very consistent and significant.
